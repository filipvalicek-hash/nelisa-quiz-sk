...
export function Screen25({ onNext, onBack, onLogoClick, onSkip, onAnswerSubmit }: Screen25Props) {
  ...
  const rightItems = [
    { id: 'A', text: 'A) U běžné inzerce jste zvyklí platit za jeden měsíc na jednom jobboardu a čekat, kdo se ozve. U Nelisy běží po stejnou dobu aktivní kampaň napříč zhruba 16 kanály, která jde za výrazně větším publikem. Součástí ceny jsou navíc služby jako firemní profil, tvorba bannerů a přehledné statistiky. Dává smysl to ověřit pilotně na jedné roli a podle výsledků se rozhodnout dál.' },
    { id: 'B', text: 'B) Právě období bez akutní potřeby je ideální chvíle řešit nábor s předstihem. Můžete budovat databázi kandidátů, posílit employer branding a mít kampaně připravené tak, aby šly rychle spustit ve chvíli, kdy potřeba reálně vznikne.' },
    { id: 'C', text: 'C) Agentury i interní marketing obvykle řeší hlavně produktové nebo brandové kampaně. Nelisa má za sebou více než 4 roky praxe a tisíce HR kampaní, ze kterých vychází specializované know-how. Díky tomu optimalizujeme rychle podle dat a oproti interní nebo agenturní správě často šetříme čas i náklady.' },
    { id: 'D', text: 'D) Proces je nastavený jednoduše – od založení profilu dokážeme spustit kampaň do 3 pracovních dnů. Díky integracím jdou kandidáti přímo do ATS, takže HR nepřibývá další nástroj ani manuální práce. Nejlepší je vyzkoušet první kampaň pilotně.' },
    { id: 'E', text: 'E) Je důležité podívat se na to, proč předchozí pokus nefungoval – často chybí správné nastavení cílové skupiny nebo optimalizace v průběhu kampaně. Díky zkušenostem s tisíci HR kampaní víme, kde bývají slabá místa a jak je včas upravit.' },
    { id: 'F', text: 'F) Jobboardy fungují primárně na aktivní kandidáty. Nelisa oslovuje i ty, kteří práci aktivně nehledají, ale mohou být otevřeni změně. Díky tomu rozšiřujete zásah mimo stejnou skupinu lidí, kterou oslovují i vaši konkurenti.' }
  ];

  // Correct mapping: 1→A, 2→B, 3→C, 4→D, 5→E, 6→F
            ...
            <div>
              ...
              <AnimatePresence>
                {isConfirmed && (
                  <ExplanationBlock>
                    <p>
                      Každá námitka otevírá prostor vysvětlit konkrétní hodnotu, kterou klient získá. Dobrá argumentace neznamená popřít klientův postřeh, ale ukázat, jak Nelisa řeší jeho skutečnou potřebu nebo minimalizuje riziko. Klíčové je vyjít z reality klienta, ne jen z produktu.
                    </p>
                  </ExplanationBlock>
                )}
              </AnimatePresence>
